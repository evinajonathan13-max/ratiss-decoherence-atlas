...
(function () {
  ...
  function numeric(value, digits = 3) { return Number.isFinite(Number(value)) ? Number(value).toFixed(digits) : "—"; }
  ...
  function drawStep() {
    ...
    dom.graphPsig.textContent = numeric(step.topology?.psig ?? step.avg_psig); dom.logicalPsig.textContent = numeric(step.logical_topology?.P_sig); const avgDec = step.qubits.length ? step.qubits.reduce((sum, node) => sum + (node.local_decoherence || 0), 0) / step.qubits.length : 0;
    ...
    dom.summary.textContent = step.decoherence_detected ? "Des zones critiques sont calculées à cette étape ; la route TSP ne sert qu’à leur inspection." : "Aucune zone critique selon le seuil exporté. Les tubes affichent uniquement les liens présents dans le JSON.";
  ...
  function updateSelected() { const step = currentStep(); const node = step?.qubits.find((item) => item.id === state.selectedId); if (!node) { dom.selected.className = "detail-list empty"; dom.selected.textContent = "Clique un qubit dans la scène pour suivre sa signature."; return; } dom.selected.className = "detail-list"; dom.selected.innerHTML = `<div><b>Qubit ${node.id}</b> · degré ${node.degree}</div><div>Fidélité simulée : ${numeric(node.fidelity_to_ideal)}</div><div>Décohérence locale : ${numeric(node.local_decoherence)}</div><div>Support topologique : ${numeric(node.topology_support ?? node.psig)}</div><div>Criticité : ${numeric(node.criticality)}</div>`; }
